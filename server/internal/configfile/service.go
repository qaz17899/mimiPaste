...
func NewService(
	...
	return &Service{sources: sources, profiles: profiles, backups: backups, active: active, fs: fs, clock: clock}
}
...
func (s *Service) Read(ctx context.Context, sourceID string) (ReadResult, error) {
	...
	return ReadResult{Source: source, Content: content, Valid: result.Valid, Error: result.Error, Fields: fields}, nil
}
...
func (s *Service) Apply(ctx context.Context, sourceID string, input ApplyInput) (ReadResult, error) {
	...
	if _, err := s.createBackup(ctx, source, &target.ID, current); err != nil {
		return ReadResult{}, err
	}
...
func (s *Service) PreviewRestore(ctx context.Context, backupID string) (DiffResult, error) {
	...
	current, err := s.readSource(item.Path)
	if err != nil {
		return DiffResult{}, err
	}
...
func (s *Service) Restore(ctx context.Context, backupID string, input RestoreInput) (ReadResult, error) {
	if !input.Confirm {
		return ReadResult{}, core.NewError(core.CodeInvalidInput, "請先確認還原備份。")
	}
	item, err := s.backups.GetBackup(ctx, backupID)
	if err != nil {
		return ReadResult{}, err
	}
	if err := s.writeSource(item.Path, item.Content); err != nil {
		return ReadResult{}, err
	}
	return s.Read(ctx, item.ConfigSourceID)
}
...
func (s *Service) writeSource(path string, content string) error {
	if err := s.fs.WriteFile(filepath.Clean(path), []byte(content), configFilePerm); err != nil {
		return core.NewError(core.CodeFileWriteError, "無法寫入設定檔，請確認路徑與權限。")
	}
	return nil
}
...
func (s *Service) createBackup(
	...
	item := backup.Backup{
		ID: core.NewID(), ConfigSourceID: source.ID, ProfileID: profileID,
		Path: source.Path, Content: content, CreatedAt: s.clock.Now(),
	}
