...
export function useProfileMutations() {
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: queryKeys.profiles.root() })
  return {
    create: useMutation({ mutationFn: createProfile, onSuccess: invalidate }),
    update: useMutation({
      mutationFn: ({ id, input }: { id: string; input: ProfileSaveInput }) => updateProfile(id, input),
